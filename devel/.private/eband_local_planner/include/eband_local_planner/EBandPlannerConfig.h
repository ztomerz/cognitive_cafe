...
namespace eband_local_planner
{
  ...
  public:
    ...
    public:
      ...
      virtual void clamp(EBandPlannerConfig &config, const EBandPlannerConfig &max, const EBandPlannerConfig &min) const override
      {
      ...
      virtual void calcLevel(uint32_t &comb_level, const EBandPlannerConfig &config1, const EBandPlannerConfig &config2) const override
      {
        if (config1.*field != config2.*field)
          comb_level |= level;
      }

      virtual void fromServer(const ros::NodeHandle &nh, EBandPlannerConfig &config) const override
      {
        nh.getParam(name, config.*field);
      }

      virtual void toServer(const ros::NodeHandle &nh, const EBandPlannerConfig &config) const override
      {
        nh.setParam(name, config.*field);
      }

      virtual bool fromMessage(const dynamic_reconfigure::Config &msg, EBandPlannerConfig &config) const override
      {
        return dynamic_reconfigure::ConfigTools::getParameter(msg, name, config.*field);
      }

      virtual void toMessage(dynamic_reconfigure::Config &msg, const EBandPlannerConfig &config) const override
      {
        dynamic_reconfigure::ConfigTools::appendParameter(msg, name, config.*field);
      }

      virtual void getValue(const EBandPlannerConfig &config, boost::any &val) const override
      {
        val = config.*field;
      }
    };
    ...
    public:
      ...
      virtual void updateParams(boost::any &cfg, EBandPlannerConfig &top) const override
      {
        PT* config = boost::any_cast<PT*>(cfg);
